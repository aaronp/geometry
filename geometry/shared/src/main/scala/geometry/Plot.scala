...
import geometry.Plot.Layout.AsciiLineImpl
...
object Plot {
  ...
  case class AsciiRows(rows: List[(Int, AsciiRow)] = Nil) {
    ...
    def render(space: Char = ' '): String = {
      val cachedMinX = minX
      val asciiRows  = rowByYCoord.view.mapValues(_.render(cachedMinX, space)).toMap.withDefault(_ => "")
  ...
  object Layout {
    ...
    implicit object AsciiPolygonImpl extends AsciiPolygon('x')
    class AsciiPolygon(char: Char) extends Layout[Polygon] {
      type UI = AsciiRows
      override def layout(value: Polygon, view: Rectangle): AsciiRows = {
        val line = new AsciiLine(char)
        value.edges.map(line.layout(_, view)).reduce(_ merge _)
      }
    }
    class AsciiRectangle(topChar: Char, rightChar: Char, bottomChar: Char, leftChar: Char, cornerChar: Char) extends Layout[Rectangle] {
      type UI = AsciiRows
      override def layout(value: Rectangle, view: Rectangle): AsciiRows = {

        val base = value.topEdge
          .ascii(topChar, view)
          .merge(value.rightEdge.ascii(rightChar, view))
          .merge(value.bottomEdge.ascii(bottomChar, view))
          .merge(value.leftEdge.ascii(leftChar, view))

        if (cornerChar == topChar) {
          base
        } else {
          base
            .merge(value.topLeft.ascii(cornerChar, view))
            .merge(value.topRight.ascii(cornerChar, view))
            .merge(value.bottomLeft.ascii(cornerChar, view))
            .merge(value.bottomRight.ascii(cornerChar, view))
        }
      }
    }
    implicit object AsciiRectangleImpl extends AsciiRectangle('-', '|', '-', '|', '+')
  }
}
